...
int main(void) {
	...
	//act_int.sa_mask를 0으로 초기화
	sigemptyset(&act_int.sa_mask);
	//act_int.sa_mask에 SIGQUIT 시그널을 추가
	sigaddset(&act_int.sa_mask, SIGQUIT);
	act_int.sa_flags = 0;

	//SIGINT 시그널이 발생했을 때의 엑션에 대한 정보 전달
	if(sigaction(SIGINT, &act_int, NULL) < 0) {
		fprintf(stderr, "sigaction(SIGINT) error\n");
		exit(1);
	}

	//act_quit의 시그널 핸들러 등록
	act_quit.sa_handler = ssu_signal_handler2;
	//act_quit.sa_mask를 0으로 초기화
	sigemptyset(&act_quit.sa_mask);
	//act_quit.sa_mask에 SIGINT 시그널을 추가
	sigaddset(&act_quit.sa_mask, SIGINT);
	act_quit.sa_flags = 0;

	//SIGQUIT 시그널이 발생했을 때의 엑션에 대한 정보 전달
